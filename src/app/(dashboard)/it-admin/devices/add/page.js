...
import { Laptop, Save, X, Package } from 'lucide-react';
...
export default function AddDevicePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    purchaseDate: '',
    warrantyExpiry: '',
    condition: 'New', // UI-only (backend enforces defaults)
    lifecycleStatus: 'Available', // UI-only (backend enforces defaults)
    location: 'Office',
    notes: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await itDeviceService.createDevice(payload);
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
        <Breadcrumb
          items={[
            ...
            { label: "Add Device", href: "/it-admin/devices/add" },
          ]}
        />
        ...
        <div className="mt-6 mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Register a new IT asset. Serial number will be auto-generated.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {formData.deviceType === 'Other' && (
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  placeholder="e.g., Keyboard, UPS, Projector, Router"
                  className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] dark:bg-gray-700 dark:text-white"
                />
              </div>
            )}
            ...
            {/* Condition and Status (Read-only for new devices) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                  Condition
