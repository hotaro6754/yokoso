...
export default function BankInfo({ data, onUpdate, updating }) {
  ...
  const renderField = (field) => {
      ...
      className: `w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-[#E0E2FE] ${
        isFieldEditable() 
          ? 'border-gray-300' 
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-[#E0E2FE]">Bank Information</h2>
        {!isEditing ? (
          <button
            onClick={() => setIsEditing(true)}
            className="px-4 py-2 text-sm bg-brand-500 text-white rounded-lg hover:bg-brand-600 transition shadow-sm hover:shadow-md font-semibold dark:bg-[#E0E2FE] dark:text-[#0B0F19] dark:hover:bg-[#BBBDEC]"
          >
            Edit Information
          </button>
        ) : (
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={updating}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-brand-500 text-white rounded-lg hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-sm hover:shadow-md font-semibold dark:bg-[#E0E2FE] dark:text-[#0B0F19] dark:hover:bg-[#BBBDEC]"
            >
              {updating ? (
                <>
      ...
      <div className="mb-4 p-3 bg-brand-50 dark:bg-brand-900/20 rounded-lg">
        <div className="flex items-start gap-2">
          <Info size={16} className="text-brand-600 dark:text-[#E0E2FE] mt-0.5 flex-shrink-0" />
          <p className="text-sm text-brand-700 dark:text-[#BBBDEC]">
          ...
          {editableBankFields.map((field) => (
            <div key={field.name} className={field.name === 'bankName' || field.name === 'accountHolderName' ? 'md:col-span-2' : ''}>
              <label className="block text-sm font-medium text-gray-700 dark:text-[#BBBDEC] mb-2">
                {field.label}
                {isFieldEditable() && (
                  <span className="text-brand-600 dark:text-[#E0E2FE] text-xs ml-2">(Editable)</span>
                )}
              </label>
              {renderField(field)}
            </div>
