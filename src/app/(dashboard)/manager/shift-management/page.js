...
export default function ShiftManagementPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'roster') {
        const res = await shiftManagementService.getMonthlyShifts(selectedMonth, selectedYear);
      ...
      if (teamMembers.length === 0) {
        const teamRes = await managerTeamService.getTeamOverview();
        // Extract teamMembers from the response structure
        const members = teamRes?.teamMembers || teamRes?.data || teamRes || [];
    ...
    <div className="p-4 sm:p-6 space-y-6 min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Manage monthly rosters and swap requests.</p>
        </div>
