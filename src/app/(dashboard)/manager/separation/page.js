...
export default function ManagerSeparationPage() {
    ...
    const fetchTeamResignations = async () => {
        try {
            setLoading(true);
            const response = await separationService.getDashboard({ status: 'PENDING' }); // Dashboard for manager
            if (response.success) {
                            ...
                            <div className="p-6 space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border dark:border-gray-700">
                                            <p className="text-sm font-medium mb-1">Reason for Resignation:</p>
                                            <p className="text-sm text-gray-600 dark:text-gray-400">"{selectedRequest.reason}"</p>
                                            {selectedRequest.reasonDetails && (
                                                <p className="text-xs text-gray-500 mt-2 italic">{selectedRequest.reasonDetails}</p>
                                            )}
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-xs font-semibold uppercase text-gray-500">Manager Retention Notes</label>
                                            <textarea
                                                rows={3}
                                                placeholder="Summary of your retention discussion with the employee..."
                                                className="w-full p-3 rounded-lg border dark:bg-gray-700 text-sm"
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {reviewData.approved ? (
                                            <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                                                ...
                                                <input
                                                    type="url"
                                                    placeholder="Link to KT doc / folder (e.g. Google Drive)"
                                ...
                                <div className="pt-6 border-t dark:border-gray-700">
                                    <button
                                        ...
                                        className={`w-full py-4 rounded-xl text-white font-bold text-lg shadow-lg transition-transform hover:scale-[1.01] ${reviewData.approved ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"}`}
                                    >
