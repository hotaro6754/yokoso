...
export default function OnboardingFlowEditPage() {
  ...
  const handleSubmit = async (values) => {
    setIsSaving(true);
    try {
      // Include companyId in the request payload
      const payload = {
        ...values,
        companyId: flow.companyId, // Ensure companyId is included
      };

      // Real API PUT request to update the flow
      const response = await apiClient.put(`/master-admin/onboarding-flow/${flowId}`, payload);
