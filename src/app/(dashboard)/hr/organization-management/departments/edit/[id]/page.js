// src/app/(dashboard)/hr/organization-management/departments/edit/[id]/page.js
"use client";
...
export default function DepartmentEditPage() {
  ...
  useEffect(() => {
    if (isItAdminView && id) {
      router.replace(`${baseOrgPath}/departments/view/${id}`);
    }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const [deptResponse, listResponse] = await Promise.all([
          organizationService.getDepartmentById(id),
          organizationService.getAllDepartments({ page: 1, limit: 100 }),
        ]);

        const dept = deptResponse.success ? deptResponse.data : deptResponse.data?.department || deptResponse.data;
        const list = listResponse.success ? listResponse.data : listResponse.data?.departments || listResponse.data || [];
        ...
        setDepartments(
          Array.isArray(list) ? list.filter((item) => String(item.id) !== String(id)) : []
        );
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await organizationService.updateDepartment(id, payload);
      toast.success("Department updated successfully");
      router.push(`${baseOrgPath}/departments/view/${id}`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-12">
      <Breadcrumb
        items={[
          ...
          { label: "Edit Department", href: `${baseOrgPath}/departments/edit/${id}` },
        ]}
        rightContent={
