// src/app/(dashboard)/hr/organization-management/designations/view/[id]/page.js
"use client";

import { useEffect, useState } from "react";
import { useParams, useRouter } from "next/navigation";
import { ArrowLeft, Edit, Briefcase, Building2, Users } from "lucide-react";
...
export default function DesignationViewPage() {
  ...
  useEffect(() => {
    const fetchDesignation = async () => {
      try {
        setLoading(true);
        const response = await organizationService.getDesignationById(id);
        const data = response.success ? response.data : response.data?.designation || response.data;
        setDesignation(data);
      } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-12 pt-4">
      <Breadcrumb
        items={[
          ...
          { label: "Designation Details", href: `${baseOrgPath}/designations/view/${id}` },
        ]}
        rightContent={
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-xl font-semibold">{designation.name}</h2>
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusTone}`}>
                      {designation.status || "INACTIVE"}
                    </span>
                  </div>
                  <p className="text-sm text-white/80">Code: {designation.code || "-"}</p>
                </div>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl bg-white/15 px-4 py-3">
                  <p className="text-[11px] uppercase tracking-wider text-white/70">Level</p>
                  <p className="text-sm font-semibold text-white">{designation.level || "-"}</p>
                </div>
                <div className="rounded-xl bg-white/15 px-4 py-3">
            ...
            <div className="rounded-2xl border border-gray-100 bg-gray-50/60 p-5 dark:border-gray-700 dark:bg-gray-900/20">
              ...
              <div className="space-y-3 text-sm text-gray-700 dark:text-gray-300">
                <div className="flex items-center gap-3">
                  ...
                  <span>Department: {designation.department?.name || "-"}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Users className="h-4 w-4 text-brand-500" />
                  <span>Org Level: {designation.orgLevel ?? "-"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-gray-50/60 p-5 dark:border-gray-700 dark:bg-gray-900/20">
              ...
              <div className="space-y-3 text-sm text-gray-700 dark:text-gray-300">
                <div className="flex items-center gap-3">
                  ...
                  <span>Designation Code: {designation.code || "-"}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Users className="h-4 w-4 text-brand-500" />
                  <span>Total Employees: {designation.employeeCount ?? 0}</span>
                </div>
              </div>
            </div>
          </div>
