...
export default function ImportRosterModal({ isOpen, onClose, onImportSuccess }) {
        ...
        if (isOpen) {
            const fetchEmployees = async () => {
                try {
                    ...
                    const members = response?.data?.employees || response?.data || response || [];
                    setEmployees(members);
                } catch (error) {
                    console.error("Failed to fetch employees for import mapping", error);
                }
            };
            fetchEmployees();
        }
    ...
    const sampleData = [
        { "Employee ID": "EMP001", "Date": "2024-03-01", "Shift Code": "S1", "Location": "Office" },
        { "Employee ID": "EMP002", "Date": "2024-03-01", "Shift Code": "S2", "Location": "Remote" }
    ];
    ...
    const exportTemplate = () => {
        try {
            ...
            saveAs(data, "hr_shift_roster_template.xlsx");
        ...
        reader.onload = async (e) => {
            try {
                ...
                jsonData.forEach((row, index) => {
                    const empCodeStr = row["Employee ID"]?.toString().trim();
                    const shiftCode = row["Shift Code"]?.toString().trim();
                    const date = row["Date"];
                    
                    if(!empCodeStr || !date || !shiftCode) {
                         errors.push(`Row ${index + 2}: Missing ID, Date or Shift Code`);
                         return;
                    }
                    
                    // Resolve Employee ID to Database ID
                    const employee = employees.find(m => 
                        m.employeeId === empCodeStr || 
                        m.id?.toString() === empCodeStr
                    );

                    if (!employee) {
                        errors.push(`Row ${index + 2}: Employee ${empCodeStr} not found`);
                        return;
                    }

                    const dbId = employee.id;

                    // Overlap Check
                    const key = `${dbId}-${date}`;
                    if (entriesHash[key]) {
                        errors.push(`Row ${index + 2}: Duplicate entry for ${empCodeStr} on ${date}`);
                        return;
                    }
                    entriesHash[key] = true;
                ...
                const result = await workforceService.importRoster(validAssignments);
