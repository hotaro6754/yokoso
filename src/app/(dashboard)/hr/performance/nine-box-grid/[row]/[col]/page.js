...
export default function NineBoxGridPositionPage() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      console.log(`Employees in box (${row},${col}):`, response.data);
  ...
  const handlePotentialChange = async (employeeId, newPotential) => {
    try {
      ...
      console.log('Attempting to update potential for employee:', employeeId);
      console.log('Current employees data:', employees);

      // Find the Nine-Box position record for this employee
      const employeePosition = employees.find(emp => emp.id === employeeId);
      if (!employeePosition) {
        console.error('Employee position not found for ID:', employeeId);
      ...
      // The ID we need is the Nine-Box position ID (this should be emp.id from the Nine-Box API response)
      const positionId = employeePosition.id;

      console.log('Using position ID:', positionId);

      // Update the backend
      console.log('Making API call to update potential...');
      const response = await performanceManagementService.updateNineBoxPositionPotential(
        positionId,
        newPotential
      );
      ...
      setEmployees(prev => prev.map(emp =>
        emp.id === employeeId
          ? { ...emp, potential: newPotential, potentialScore: potentialScores[newPotential] }
          : emp
      ));
  ...
  const handleBackToGrid = () => {
    router.push('/hr/performance/nine-box-grid');
  };
        ...
        {filteredEmployees.length > 0 ? (
                ...
                <div key={employee.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-5 hover:shadow-md transition-shadow">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between">
                      ...
                      <select
                        value={displayData.potential}
                        onChange={(e) => handlePotentialChange(employee.id, e.target.value)}
                        className="text-xs px-3 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                      >
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              {searchTerm ? 'No employees match your search criteria.' : 'There are no employees in this Nine-Box position.'}
            </p>
          </div>
        )}
      </div>
